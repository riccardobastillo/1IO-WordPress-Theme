...
class Less_Parser{
	...
	private function GetRules( $file_path ){
		...
		if( $cache_file ){
			if( Less_Parser::$options['cache_method'] == 'callback' ){
				...
				switch(Less_Parser::$options['cache_method']){
					...
					case 'serialize':
						$cache = unserialize(fl_theme_filesystem()->fl_file_get_contents($cache_file));
						if( $cache ){
							touch($cache_file);
							$this->UnsetInput();
							return $cache;
		...
		if( $cache_file ){
			if( Less_Parser::$options['cache_method'] == 'callback' ){
				...
				switch(Less_Parser::$options['cache_method']){
					case 'serialize':
						fl_theme_filesystem()->fl_file_put_contents( $cache_file, serialize($rules) );
						break;
					case 'php':
						fl_theme_filesystem()->fl_file_put_contents( $cache_file, '<?php return '.self::ArgString($rules).'; ?>' );
						break;
					case 'var_export':
						//Requires __set_state()
						fl_theme_filesystem()->fl_file_put_contents( $cache_file, '<?php return '.var_export($rules,true).'; ?>' );
						break;
				}
	...
	public function SetInput( $file_path ){
		...
		if( $file_path ){
			$this->input = fl_theme_filesystem()->fl_file_get_contents( $file_path );
		}
