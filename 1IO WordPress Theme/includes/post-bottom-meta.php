...
if ( $show_full || is_single() ) {
	...
	if ( $show_cats || $show_tags ) {
		...
		if ( $show_cats && $cats ) {
			...
			printf( _x( 'Posted in %s', 'Post meta info: category.', '1io' ), $cats );
		}

		if ( $show_tags && $tags ) {
			if ( $show_cats && $cats ) {
				/* translators: %s: Continuing of the sentence started with Posted in Category */
				printf( _x( ' and tagged %s', 'Post meta info: tags. Continuing of the sentence started with "Posted in Category".', '1io' ), $tags );
			} else {
				/* translators: %s: Post meta info tags */
				printf( _x( 'Tagged %s', 'Post meta info: tags.', '1io' ), $tags );
			}
		}
	...
	if ( $comments && ! is_single() ) {
		comments_popup_link( _x( 'Leave a comment', 'Comments popup link title.', '1io' ), __( '1 Comment', '1io' ), _nx( '1 Comment', '% Comments', get_comments_number(), 'Comments popup link title.', '1io' ) );
	}
