...
class Less_Cache{
	...
	public static function Get( $less_files, $parser_options = array(), $modify_vars = array() ){
		...
		if( !empty($modify_vars) ){
			...
			if( !file_exists($vars_file) ){
				fl_theme_filesystem()->fl_file_put_contents($vars_file, $lessvars);
			}
		...
		$list[] = $compiled_name;
		$cache = implode("\n",$list);
		fl_theme_filesystem()->fl_file_put_contents( $list_file, $cache );


		//save the css
		fl_theme_filesystem()->fl_file_put_contents( $output_file, $compiled );
	...
	static function ListFiles($list_file, &$list, &$css_file_name ){
		...
		$list = explode("\n",fl_theme_filesystem()->fl_file_get_contents($list_file));
